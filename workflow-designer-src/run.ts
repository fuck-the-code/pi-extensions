...
export async function verifyNodeGoal(
	...
	const resolvedInputs = resolveNodeInputs(cwd, node, run, runDir);
	const verifierAgent = node.verification?.agent;
	const verifierPrompt = node.verification?.prompt;
